...
function HeroCarousel() {
  ...
  return (
    <section className="hero-section" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
              ...
              <div className="hero-caption">
                ...
                <div className="hero-dots">
                  {heroSlides.map((dotSlide, dotIndex) => (
                    <button key={dotSlide.id} type="button" aria-label={`Go to slide ${dotIndex + 1}`} className={`hero-dot${dotIndex === activeSlide ? " hero-dot-active" : ""}`} onClick={() => goToSlide(dotIndex)} />
                  ))}
                </div>
              </div>
            </div>
  ...
  return (
    <div className="odyssey-card">
      <div className="odyssey-card-inner" style={{ borderColor: color }}>
        ...
        <div className="odyssey-card-icon-ring">
  ...
  return (
    <section className="odyssey-section">
      <div className="odyssey-heading-block">
        ...
        <button type="button" className="odyssey-demo-button">
          Book a demo
        </button>
      </div>
      ...
      <div className={`odyssey-extra-wrapper${isExpanded ? " odyssey-extra-open" : ""}`}>
      ...
      <button type="button" className="odyssey-toggle-button" onClick={() => setIsExpanded((current) => !current)} aria-expanded={isExpanded}>
        <span className="odyssey-toggle-pill">
          <span className="odyssey-toggle-label">{isExpanded ? "View Less" : "View More"}</span>
        </span>
        <span className="odyssey-toggle-badge">
          <span className="odyssey-toggle-badge-inner">{arrowAnimationData && <Lottie animationData={arrowAnimationData} loop autoplay className={`odyssey-toggle-icon${isExpanded ? " odyssey-toggle-icon-flipped" : ""}`} />}</span>
        </span>
      </button>
    </section>
  );
...
const storyMissionVisionCards: StoryMissionVisionCardData[] = [
  ...
  },
  {
    ...
    image: "/images/student-online-cute-guy-checked-shirt-with-glasses-studying-computer-showing-good-gesture.jpg",
    accentColor: "#2A7308",
    Icon: GoalIcon,
  },
  {
...
function StoryMissionVisionCard({ title, description, image, accentColor, Icon }: StoryMissionVisionCardData) {
  return (
    <div className="smv-card">
      <div className="smv-card-icon-ring">
...
function PlatformStat({ value, label, accentColor, Icon }: PlatformStatData) {
  return (
    <div className="smv-stat">
      <div className="smv-stat-icon-ring" style={{ borderColor: accentColor }}>
  ...
  return (
    <section className="smv-section">
      <div className="smv-heading-block">
        ...
        <h2 className="smv-heading">Our Story! Our Mission! Our Vision</h2>
      ...
      <div className="platform-overview-inner">
        <div className="platform-overview-media">
          <div className="platform-overview-media-frame">
            <video className="platform-overview-video" src="/videos/6561564-uhd_3840_2160_25fps.mp4" autoPlay muted loop playsInline controls />
          </div>
        </div>

        <div className="platform-overview-content">
          <span className="platform-overview-tag">Platform Overview</span>
          <h2 className="platform-overview-heading">NeuroLXP One Platform! Many Missions! One Future</h2>
          <div className="platform-overview-list">
            {platformOverviewItems.map((item, index) => (
              <div key={item.id} className="platform-overview-list-item">
                <div className="platform-overview-number-badge" style={{ background: item.color }}>
                  {index + 1}
                </div>
                <div className="platform-overview-capsule">
                  <div className="platform-overview-icon-circle" style={{ color: item.color }}>
...
function PlatformPurpose() {
  ...
  return (
    <section className="platform-purpose-section">
      <h2 className="platform-purpose-heading">One Platform! Every Learning and Skilling Purpose</h2>
    ...
    <section className="meet-neurolxp-section">
      <div className="meet-neurolxp-frame">
        ...
        <div className="meet-neurolxp-text">
          ...
          <h2 className="meet-neurolxp-heading">Next-Generation Neomorphism learning Platform</h2>
...
function WhyChooseCard({ title, description, dashColor, iconBg, Icon }: WhyChooseCardData) {
  return (
    <div className="why-choose-card">
      <div className="why-choose-icon-ring">
...
function ArrowLeftIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 47 47" fill="none" className={className}>
  ...
  return (
    <section className="learning-modules-section">
      <div className="learning-modules-heading-block">
        ...
        <h2 className="learning-modules-heading">
    ...
    <div className="testimonial-card-wrapper">
      ...
      <div className="testimonial-card" style={{ borderRightColor: accentColor }}>
        <div className="testimonial-rating-badge">
          <span className="testimonial-rating-number">5</span>
    ...
    <div className="faq-item">
      <button type="button" className="faq-item-header" onClick={onToggle} aria-expanded={isOpen}>
        <div className="faq-item-header-left">
          <span className="faq-item-triangle" style={{ borderLeftColor: item.colorStart }} />
          <span className="faq-item-number" style={{ backgroundImage: `linear-gradient(180deg, ${item.colorStart} 0%, ${item.colorEnd} 100%)` }}>
            {item.number}
          </span>
          <span className="faq-item-divider" style={{ backgroundImage: `linear-gradient(180deg, ${item.colorStart} 0%, ${item.colorEnd} 100%)` }} />
          <span className="faq-item-title">{item.title}</span>
        </div>
        <span className="faq-item-toggle" style={{ color: item.colorStart }}>
          {isOpen ? <MinusIcon className="faq-item-toggle-icon" /> : <PlusIcon className="faq-item-toggle-icon" />}
        </span>
      </button>
      <div className={`faq-item-answer-wrapper${isOpen ? " faq-item-answer-open" : ""}`}>
